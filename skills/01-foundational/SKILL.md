...
### 1. State
A plain C# class marked with the `[State]` attribute (inheriting from `State`) that represents the abstract state of your system. This is NOT your real database — it's the simplest possible representation of what your system "knows."
...
├── State ([State] attribute)          // Abstract model of system state
...
- `Accordant` — Core types: `State`, `Spec<>`, `Operation<>`, `Expect`, `ExpectedOutcomes`
...
1. **Design State** → Define `State` subclass with `[State]` attribute with properties modeling system state
