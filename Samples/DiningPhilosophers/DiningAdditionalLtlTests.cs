namespace DiningPhilosophers
{
    ...
    public class DiningAdditionalLtlTests
    {
        ...
        private static readonly Fairness PhilFairness =
            Fairness.Strong(sf => sf is Dining.PhilStep);

        [SetUp]
        public void Setup()
        {
            _naiveRoot = StateGraph.ExploreStateGraph(Dining.AllSteps(asymmetric: false), Dining.InitialState(), lazy: true);
            _asymRoot = StateGraph.ExploreStateGraph(Dining.AllSteps(asymmetric: true), Dining.InitialState(), lazy: true);
        }
