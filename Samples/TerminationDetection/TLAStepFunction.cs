...
namespace TerminationDetection
{
    ...
    /// <summary>
    /// Step functions in Accordant are _consumed_ after they are applied to a given state.
    ...
    /// step functions in Accordant) but the act of applying them to a state does not
    /// _consume_ them. Unlike TLA+, step functions in Accordant can also _produce_ zero or
    ...
    /// <see cref="BaseStepFunction.Apply(Accordant.State)"/> operation, thus producing the
