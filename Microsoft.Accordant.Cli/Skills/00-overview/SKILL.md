...
| **Define state** | `design-state` | `[State]` attribute, collections, `[SharedState]`, nested state, initialization |
...
Model the abstract state of your system using the `[State]` attribute. Keep it minimal — only what matters for observable behavior.
