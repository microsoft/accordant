...
The key question: **Does the framework need to be told how to construct this request from prior operation data?** If yes, write a derivation. If the request can be constructed independently without any prior operation data, you don't need one. Note that derivation functions are only needed if you're using the framework's test execution capabilities — they are not needed if you're constructing the test cases yourself and only using the spec for conformance testing via its `Allows` or `AllowsConcurrent` methods.