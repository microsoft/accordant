...
| [WalProcessCoroutines](https://github.com/microsoft/accordant/tree/main/Samples/WalProcessCoroutines) | **Experimental:** the same write-ahead log written as *processes* — independently active replay coroutines (an external client, a guarded request-handler launch, a page writer and a recovery worker) composed by a scheduler with a structurally-registered server failure domain whose crash discards the domain's continuations. The local handler reads as ordinary sequential code (append → flush → acknowledge) with no protective guards; a crash kills it and recovery reports in its place. Adds a guarded `When`/`WaitUntil` wait, a `FailureDomain`-scoped `On` launch to the coroutine runtime, and refines the guarded-action atomic store on safety and the strong recovery/report fairness ladder. Not a supported API — see [Model-Checking Frontends](concepts/model-checking-frontends.md). |
