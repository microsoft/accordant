...
| [WalProcessCoroutines](https://github.com/microsoft/accordant/tree/main/Samples/WalProcessCoroutines) | **Experimental:** the same write-ahead log written as *processes* — independently active replay coroutines (an external client, a guarded request-handler launch, a page writer, a reporter and a recovery worker) composed by a scheduler with a server failure domain whose crash discards the server-domain continuations. Adds a guarded `When`/`WaitUntil` wait and an `On` launch to the coroutine runtime, and refines the guarded-action atomic store on safety and the strong recovery/report fairness ladder. Not a supported API — see [Model-Checking Frontends](concepts/model-checking-frontends.md). |
