...
### Why the [State] attribute?

The `[State]` attribute activates a source generator that provides:
- **Automatic cloning** (deep copy via generated code)
...
1. **Define State** - `State` subclass with `[State]` attribute tracking what matters
